...
def math(name):
    db = pymysql.connect(host='localhost', port=3306, user='Math', passwd='0492932899', charset='utf8', db='Math')
    sql = """SELECT * FROM `用戶端` WHERE `姓名` LIKE '""" + name + "'"
...
@app.route("/Search/Math")
def Math():
    return render_template("Math查詢入口.html")
...
@app.route("/Search/Math/Search" ,methods = ["POST"])
def Ma_User_chack_data():
    user_name = request.form["name"]
    user_name = str(user_name)
    ...
    try:
        data = math(user_name)
        print("YES")
        return render_template("Math學生資料查詢結果.html" ,Class =  data[0] ,class_mt = data[1] ,teacher_en = data[2] ,no = data[3] ,id = data[4] ,name = data[5])
    
    except:
        return render_template("Math學生資料查詢結果.html" ,Class =  "無資料" ,class_mt = "無資料" ,teacher_en = "無資料" ,no = "無資料" ,id = "無資料" ,name = "無資料")
...
class MAIN():
    ...
    def examination(name ,file_path ,web_path):
        ...
        for (english_word, chinese_word) in random.sample(word, question):
            times += 1
            return render_template("單字王測驗.html" ,no = times ,chinese = chinese_word ,path = web_path ,ans = answer)
...
@app.route("/")
def Loading():
    return render_template("線上解題網.html")
...
@app.route("/Vocabulary/User/Download/Windows/FOV9")
def User_downland_windows_FOV9():
    Login.verify_user()
    return render_template("單字王Windows桌面版下載.html")
...
def User_text_error_show():
    ...
    '''with open("./Vocabulary/Error_download_" + user_name + ".txt" ,mode='a' ,encoding="utf-8") as Error_download:
